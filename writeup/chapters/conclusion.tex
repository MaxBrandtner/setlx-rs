...
Several test cases expose further correctness issues. Currently, mutating a class member \texttt{x} in the constructor, by accessing it through the \texttt{this} primitive doesn't mutate the object member if a parameter of the same name \texttt{x} exists. In the reference implementation mutation through accessing \texttt{this} only fails to propagate to the final value of the object member, if \texttt{x} is mutated directly beforehand. Furthermore, in the reference implementation, mutating a static class value also mutates the value for any object of the class that hasn't mutated the value before, while no memory is shared between a class and objects of the class in this implementation. Ambiguous map accessing yields \texttt{om} in the reference implemtation, while in this rewrite, the first match is returned instead. Furthermore, in this implementation term evaluation is currently only supported for expressions, but not for statements. As this interpreter doesn't support rational numbers, the factorial operation isn't implemented for fractions.

\subsection{Completeness}

A number of predefined procedures remain unimplemented. The most straightforward to add are general-purpose string and collection procedures such as \texttt{canonical}, \texttt{rational}, \texttt{sort}, \texttt{shuffle}, \texttt{trim}, \texttt{toLowerCase}, \texttt{toUpperCase}, \texttt{startsWith}, \texttt{endsWith}, \texttt{collect}, \texttt{permutations}, \texttt{nextPermutation}, \texttt{getTerm}, \texttt{getScope}, and \texttt{isMap}.
...
Beyond optimization passes, several structural changes would improve baseline performance. A dedicated load instruction would eliminate the per-instruction conversion from \texttt{IRValue} to \texttt{InterpVal}. Making \ac{IR} instructions single-purpose and replacing the builtin procedure enum with dispatchable function pointers would reduce branching and improve branch predictor effectiveness. Together, these changes should substantially increase the performance of the interpreter, before any \ac{JIT} compilation is introduced.
